...
const MarketCapSummary = ({ liveRawCards }: MarketCapSummaryProps) => {
  ...
  const segments = [
    { label: `Raw Cards (${rawData.length.toLocaleString()}+)`, items: rawData, color: "bg-terminal-green" },
    { label: `Graded Cards (${gradedCards.length.toLocaleString()}+)`, items: gradedCards, color: "bg-terminal-amber" },
    { label: `Sealed Products (${sealedProducts.length.toLocaleString()}+)`, items: sealedProducts, color: "bg-terminal-blue" },
  ];
