...
/** Deterministic multiplier per time window so each tab looks different */
const getTimeMultiplier = (card: CardData, category: TimeCategory): number => {
  const seed = card.name.length + card.market;
  switch (category) {
    case "race": return 0.15 + (Math.sin(seed * 1.3) * 0.1);
    case "hour": return 0.4 + (Math.sin(seed * 2.1) * 0.15);
    case "day": return 1;
    case "week": return 2.5 + (Math.sin(seed * 0.7) * 0.8);
  }
};

const getCardsForCategory = (cards: CardData[], category: TimeCategory): (CardData & { adjustedChange: number; adjustedPrice: number; prevPrice: number; inventory: number })[] => {
  return [...cards]
    .map((card) => {
      ...
      return { ...card, adjustedChange, adjustedPrice: card.market, prevPrice, inventory };
    })
  ...
  return (
    ...
    >
      ...
      <div className="border-b border-border/60 px-4 py-2.5 flex items-center justify-between gap-2">
        <h2 className="text-xs font-bold tracking-wide text-secondary uppercase font-display">{title}</h2>
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-border">
              ...
              <th className="px-1.5 py-1.5 text-left font-mono text-[9px] tracking-widest text-muted-foreground uppercase">Card</th>
              <th className="px-1.5 py-1.5 text-right font-mono text-[9px] tracking-widest text-muted-foreground uppercase hidden sm:table-cell">Inv</th>
              <th className="px-1.5 py-1.5 text-right font-mono text-[9px] tracking-widest text-muted-foreground uppercase">Price</th>
              <th className="px-1.5 py-1.5 text-right font-mono text-[9px] tracking-widest text-muted-foreground uppercase">% Move</th>
            </tr>
          </thead>
          <tbody>
            {sorted.map((card, i) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-1.5 py-1.5">
                    ...
                    <span className="font-mono text-xs text-foreground font-medium block truncate max-w-[120px]">{card.name}</span>
                  ...
                  <td className="px-1.5 py-1.5 text-right hidden sm:table-cell">
                    <span className="font-mono text-[10px] font-semibold text-foreground">{card.inventory}</span>
                  </td>
                  <td className="px-1.5 py-1.5 text-right">
                    <span className="font-mono text-xs font-bold text-foreground block">
                      ${card.adjustedPrice.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                    </span>
                    <span className={`font-mono text-[9px] ${priceDelta >= 0 ? "text-terminal-green" : "text-terminal-red"}`}>
                      {priceDelta >= 0 ? "+" : ""}${Math.abs(priceDelta).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </span>
                  </td>
                  <td className="px-1.5 py-1.5 text-right">
                    <span className={`inline-flex items-center gap-0.5 font-mono text-[11px] font-black px-1 py-0.5 rounded ${
                      card.adjustedChange >= 0
                        ? "text-terminal-green bg-terminal-green/10"
                        : "text-terminal-red bg-terminal-red/10"
                    }`}>
                      {card.adjustedChange >= 0 ? "▲" : "▼"} {Math.abs(card.adjustedChange).toFixed(2)}%
                    </span>
